...
if(isset($_GET['action'])) {
	...
	elseif($action=="saveadd" || $action=="savemodify" ||$action=="saveaddmore") {
		...
		if($action=="saveadd"){
			...
			if($sn_t['SN'])
			{
				WriteErrMsg("<br><li>This Serial Number exist (SN):".$sn."</li>");
				exit();
			}
		}	
		//$ErrMsg="";
...
$updatesql_c.=",SN='".$sn."'";
  ...
  $insertsql2_c.="','".$_POST['AUTOCFG_KEY'];
	...
	if($action=="saveadd"){
		...
		WriteSuccessMsg("<br><li>Add Success!</li>".$action,"{$URL}?job=productmanage&classid=".$values['classid']);
	}
	elseif($action=="saveaddmore"){
		...
		for($i=0;$i<$sncount;$i++){
			...
			if($sn_t['SN'])
			{
				$errmsg.="<br><li>This Serial Number exist (SN):".$newsn."</li>";
				continue;
			}
			...
			$succmsg.="<br><li>Add Success! (SN):".$newsn."</li>";
		}
		if(!$errmsg)
		...
		WriteSuccessMsg("<br><li>Modify Success!</li>".$action,"{$URL}?job=productmanage&classid=".$values['classid']);
	}
//	$query=$db->query($insertsql);
	...
	elseif($action=="del") {
		...
		WriteSuccessMsg("<br><li>Delete Success!</li>","{$URL}?job=productmanage&classid={$classid}");
	}
