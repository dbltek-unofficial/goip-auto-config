...
if(isset($_GET['action'])) {
	...
	if($action=="modify") {
		...
		if(empty($orderid))
			$ErrMsg=$ErrMsg."<br><li>Serial name can not be empty and must be digit</li>";
		if(empty($miaoshu))
			$ErrMsg=$ErrMsg."<br><li>Serial definition file can not be empty</li>";
		if($ErrMsg!="")
		{
				//WriteErrMsg($ErrMsg);
				echo $ErrMsg;
 ...
 USER varchar(80) NOT NULL default '',";
					...
					WriteSuccessMsg("<br><li>Add serial success</li>","{$URL}?job=projectclass");
				}
				else{
					$ErrMsg=$ErrMsg."<br><li>Serial[$classname]is exist!</li>";
					WriteErrMsg($ErrMsg);
				}
			}
	 ...
	 elseif($action=="savemodify") {
		...
		$ErrMsg="";
		if(empty($classname))
			$ErrMsg ='<br><li>Serial name can not be empyt!</li>';
		if(empty($orderid))
			$ErrMsg=$ErrMsg."<br><li>Serial sort can not be empty and moust be digit</li>";
		if(empty($miaoshu))
			$ErrMsg=$ErrMsg."<br><li>Serial definition file can not be empty</li>";
		if($ErrMsg!="")
				WriteErrMsg($ErrMsg);
			...
			$query=$db->query("SELECT classname FROM ".$tablepre."projectclass WHERE classid='$classid' ");
				$rs=$db->fetch_array($query);
				if(empty($rs[0]))
					WriteErrMsg("<br><li>This serial is not exist!</li>");
				else{
					if($classname!=$rs[0]){
						$query=$db->query("SELECT classid FROM ".$tablepre."projectclass WHERE classname='$classname' ");
						$rs=$db->fetch_array($query);
						if(empty($rs[0])){
							...
							WriteSuccessMsg("<br><li>Modify[$classname]serial success!</li>","{$URL}?job=projectclass");
						}
						else{
							$ErrMsg=$ErrMsg."<br><li>[$classname]Serial is exist!</li>";
							WriteErrMsg($ErrMsg);
						}
					}
					else{
	 ...
	 elseif($action=="del") {
		...
		$ErrMsg="";
		if(empty($classid))
			$ErrMsg ='<br><li>Please choose one at least to delete the object</li>';
		if($ErrMsg!="")
			WriteErrMsg($ErrMsg);
		else{
			...
			if(empty($rs[0]))
				...
				//$db->query("DELETE FROM ".$tablepre."projectclass  WHERE classid IN ($classid)");
				WriteSuccessMsg("<br><li>delete[$classname]serial success!</li>","{$URL}?job=projectclass");
			}
		}
	}
	else
