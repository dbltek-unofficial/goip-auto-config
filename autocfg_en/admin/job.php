...
if(isset($_GET['action'])) {
	...
	switch($action) {
		...
		case 'News_Class_Modify':
			...
			if(empty($ClassName))
				$ErrMsg ='<br><li>column title shouldn’t be empty</li>';
			if(empty($OrderId))
				$ErrMsg=$ErrMsg."<br><li>sort order shouldn’t be empty and only for number</li>";
			if($ErrMsg!="")
				WriteErrMsg($ErrMsg);
			else{
		...
		case 'edit':
					...
					if(empty($Content))
						$ErrMsg=$ErrMsg."<br><li>Content shouldn’t be empty</li>";
					if($ErrMsg!="")
						WriteErrMsg($ErrMsg);
					else
					{
		...
		case 'order':
			...
			WriteSuccessMsg("<br><li>sort order成功</li>","{$URL}?job=Product_Class&action=order");
			break;
		case 'ModifyMyPwd':
					...
					$ErrMsg="";
					if(empty($password))
						$ErrMsg ='<br><li>Your password shouldn’t be empty</li>';
					if($ErrMsg!="")
						WriteErrMsg($ErrMsg);
		...
		case 'Admin_Add':
					...
					$ErrMsg="";
					if(empty($username))
						$ErrMsg ='<br><li>Your username shouldn’t be empty</li>';
					if(empty($password))
						$ErrMsg ='<br><li>Your password shouldn’t be empty</li>';
					if($ErrMsg!="")
						WriteErrMsg($ErrMsg);
						...
						if(empty($rs[0])){$password=md5($password);
							$query=$db->query("INSERT INTO ".$tablepre."admin (UserName,Password) VALUES ('$username','$password')");
