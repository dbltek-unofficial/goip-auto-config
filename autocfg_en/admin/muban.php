...
if(isset($_GET['action']))
{
	...
	if($action=="add")
	{
		...
		if($muban==""){
			WriteErrMsg("<br><li>There must be at least one serial or one configuration file exist!</li>");
			exit();
		}
	 ...
	 elseif($action=="saveadd" || $action=="modifysave") {
		...
		$ErrMsg="";
		if(empty($projectid))
			$ErrMsg ='<br><li>Series name were not be empty!</li>';
		if(empty($title))
			$ErrMsg=$ErrMsg."<br><li>Template were not be empty!</li>";
		...
		{
			WriteErrMsg("<br><li>The template already exist in this serial, please change a new one</li>");
			exit();
		}
		if($action=="saveadd")
		{
		...
		{
			WriteErrMsg("<br><li>Can not change initial template name:</li>");
			exit();
		}
//	print_r($values);
//	$values['TELPORT'] = 2;
...
*/
	...
	if($action=="saveadd"){
		...
		WriteSuccessMsg("<br><li>Add Template success</li>".$action,"{$URL}?job=muban");
	}
	else {
		...
		WriteSuccessMsg("<br><li>modify Template success</li>".$action,"{$URL}?job=muban");
	}
...
*/
	elseif($action=="del") {
		...
		if(empty($id)||empty($classid))
			$ErrMsg ='<br><li>Please choose the delete object</li>';
		if($ErrMsg!="")
			WriteErrMsg($ErrMsg);
		else{
			...
			$rs=$db->fetch_array($query);
			if(empty($rs[0]))
				WriteErrMsg("<br><li>This Series do not exist!</li>");
			else{
				$muban=$rs[0];
				$remuban=$db->query("SELECT muban,title FROM ".$rs['muban']." WHERE id='$id' ");
				$rsmuban=$db->fetch_array($remuban);
				if(empty($rsmuban[0]))
					WriteErrMsg("<br><li>This template do not exist!</li>");
				else if($rsmuban['muban']==$rsmuban['title'])
					WriteErrMsg("<br><li>Can not delete inital template:[".$rsmuban['muban']."]！</li>");
				else{
					...
					WriteSuccessMsg("<br><li>delete[".$rsmuban['muban']."]template success</li>","{$URL}?job=muban");
				}
			}
		}
	}
