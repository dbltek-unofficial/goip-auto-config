...
<table wIdth="100%" border="0" align="center" cellpadding="0" cellspacing="1" class="border">
  ...
  <tr class="tdbg" onmouseout="this.style.backgroundColor=''" onmouseover="this.style.backgroundColor='#BFDFFF'">
    <td valign="middle"><ul>
        <li>The function in restoring backup data at the same time, all the coverage of the original data, to determine whether there is a need to restore, avoid data loss
        <li>Data Recovery can restore function of the system is derived from the data files, may not be able to identify other software derived format
        <li>Data recovery needs support from the local server and uploading files to ensure data upload size smaller than the maximum allowed. Otherwise, we will resume the use of server</li>
		<li>If you use a sub-volume backup, only manual documents into one volume, other data from documents into the system automatically</li>
   	 </ul>
	</td>
  </tr>
</table>
...
{global $d;
$sqls=file($fname);
if(array_shift($sqls)!="autocfg_en v1.1\n")
	return false;
foreach($sqls as $sql)
	{
	str_replace("\r","",$sql);
